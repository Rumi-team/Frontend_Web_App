    ...
    >
      ...
      {toggle !== undefined && (
        <div
          ...
          aria-checked={toggle}
          aria-label={title}
          className={`relative h-6 w-11 rounded-full transition-colors ${toggle ? "bg-blue-500" : "bg-gray-200 dark:bg-gray-600"}`}
        >
          <div
      ...
      {chevron && <ChevronRight className="h-4 w-4 text-gray-300 dark:text-gray-600" />}
    </button>
  )
}
...
function SectionHeader({ title }: { title: string }) {
  return (
    <p className="px-4 pt-6 pb-2 text-xs font-semibold uppercase tracking-wider text-[#7B8F6B] dark:text-[#9BB08B]">
      {title}
    </p>
  )
}
  ...
  return (
    <div className="min-h-dvh pb-24" style={{ background: "var(--app-bg, #FAF8F3)" }}>
      ...
      <div className="mx-4 rounded-2xl bg-white dark:bg-gray-900 overflow-hidden shadow-sm dark:shadow-gray-800/20">
        ...
        <div className="mx-4 border-t border-gray-100 dark:border-gray-800" />
        <SettingsRow icon={<Users className="h-5 w-5" />} title="Use the App Alongside a Human Life Coach" description="Pair your AI coaching with a certified life coach" onClick={() => setShowHumanCoach(true)} chevron />
      </div>
